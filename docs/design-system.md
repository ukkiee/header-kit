...
header-kit UI 테마의 토큰 출처. 한 제품 마케팅 사이트의 디자인 언어를 분석해 브랜드 중립적으로 정리한 참조 스펙이다. 실제 코드에 적용된 부분(accent #0066cc, 중립 램프, 시스템 폰트 스택, scale-95 press)은 `src/app/styles/global.css`의 `@theme` + `src/ui/` 프리미티브에 있다. 이 문서는 마케팅 레이아웃까지 포함한 전체 참조이며, 확장에는 토큰 시스템만 밀도에 맞춰 채택했다.

> **shadcn/ui 채택 이후 갈라진 것** (ADR 0014). 프리미티브를 shadcn 소스로 교체하면서 이 참조 스펙과 실제 구현이 두 곳에서 어긋난다. 소스를 고치지 않기로 한 대가이고, 알고 받아들인 것이다.
>
> - **버튼 모서리** — 주 액션의 pill(`rounded-full`) 문법이 사라졌다. shadcn Button은 변형과 무관하게 같은 모서리를 쓴다. 나란히 선 취소·저장이 서로 다른 모양이던 문제는 이로써 없어졌다(예전에는 축을 따로 두어 맞췄다).
> - **팝업 그림자** — 아래 Elevation의 "무그림자" 원칙에서 Select 팝업이 빠진다. shadcn Content는 `shadow-md ring-1`로 떠 있다. 카드·사이드바·배너는 여전히 보더+명도만 쓴다.
>
> 유지된 것: 단일 blue accent(두 번째 브랜드 색 없음), near-black 본문(#1d1d1f), 시스템 폰트 스택, 누름·호버 spring scale, 포커스 링 단일 토큰.

## Overview

This reference design is a masterclass in **reverent product photography framed by near-invisible UI**. Every page is a stack of edge-to-edge product "tiles" — alternating light and dark canvases, each centered on a hero headline, a one-line tagline, two tiny blue pill CTAs, and an impossibly crisp product render. Nothing competes with the product. Typography is confident but quiet; color is either pure white, an off-white parchment, or a near-black tile; interactive elements are a single, quiet blue.

Density is unusually low even by contemporary SaaS standards. Each tile occupies roughly one viewport, and there is no decorative chrome — no borders, no gradients, no decorative frames, no shadows on headlines. Elevation appears only when a product image rests on a surface (a single soft `rgba(0, 0, 0, 0.22) 3px 5px 30px` drop for visual weight). The result is a catalog that feels more like a museum gallery: the wall disappears and the artifact takes over.

Store and shop surfaces retain the same chassis but switch modes. The product configurator (flagship product, accessories grid) introduces a tight grid of white utility cards at `{rounded.lg}` (18px) radius with a thin border, paired with a persistent thin sub-nav strip. The environment page leans darker and more editorial. Across all five surfaces the typographic system, spacing rhythm, and the single blue accent are consistent — this is one design language expressed at different volumes.
...
- the display typeface + the text typeface — negative letter-spacing at display sizes for the signature "tight" headline feel.
...
> **Source pages analyzed:** homepage, environment, store, flagship product buy page, accessories index. The color system is identical across all five surfaces; only the surface-mode mix differs.

### Brand & Accent
- **Action Blue** (`{colors.primary}` — #0066cc): The single brand-level interactive color. All text links, all blue pill CTAs ("Learn more", "Buy"), and the focus ring root. This is the reference design's quiet but universal "click me" signal. Press state shifts to a slightly darker variant via the active scale transform rather than a hex change.
...
- **Parchment** (`{colors.canvas-parchment}` — #f5f5f7): The signature off-white. Used for alternating light tiles, footer region, and the default page canvas in store utility sections. Just different enough from white to create rhythm.
...
- **Body** (`{colors.body}` — #1d1d1f): Same hex as ink — the reference design uses one near-black tone for all text on light surfaces.
...
### Brand Gradient
**No decorative gradients.** Atmospheric depth on product photography (the flagship product camera plate, the watch straps, earbuds reflections) is inherent to the imagery, not a CSS gradient overlay. The environment page's hero uses photographic atmosphere (mountain vista at dawn) but no gradient tokens are defined. This is a rare design language with zero gradient-based design tokens.

## Typography

### Font Family
- **Display / Body**: the platform **system UI font stack** — `system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif`. On each OS this resolves to the native system typeface, crisp at UI sizes, so no webfont is bundled. Display sizes lean on tight tracking rather than a bespoke face.
...
| `{typography.display-md}` | 34px | 600 | 1.47 | -0.374px | Section heads (the text typeface at display proportions) |
...
- **Body copy at 17px, not 16px.** the reference design breaks the SaaS convention and runs paragraph text at 17px. The extra pixel gives the page an unmistakable "reading, not scanning" pace.
...
- **Weight 600, not 700, for headlines.** the reference design's headlines sit at weight 600. Weight 700 is used sparingly for `{typography.tagline}` (21px) when a touch more assertion is needed.
...
The system stack resolves to the native OS UI font. When a consistent cross-platform face is needed:

- Use `system-ui, -apple-system, BlinkMacSystemFont` as the first stack entries — on macOS/iOS this resolves to the native system typeface.
- For a bundled cross-platform equivalent, **Inter** (open-source, variable) is the closest match. Inter at weight 600 with `font-feature-settings: "ss03"` approximates the rounded "a" of the native face.
- Nudge `letter-spacing` down by `-0.01em` on display sizes to re-create the tight feel; Inter's default tracking runs slightly wider.
...
the reference design's whitespace is the product's pedestal. Every tile begins with at least 64px of air above its headline and 48–64px below. Product renders are never crowded; the nearest content to a product image is at least 40px away. The footer is the only area that breaks this — there, the reference design goes deliberately dense to make the full information architecture visible at a glance.
...
**`button-icon-circular`** — Floats over photography. 44 × 44px, background `{colors.surface-chip-translucent}` at ~64% alpha, icon in `{colors.ink}`, rounded `{rounded.full}`. Used for carousel controls, close buttons, and in-image controls (product image thumbnails on the product buy page).
...
**`configurator-option-chip`** — Pill-shaped tappable cell used in the flagship product buy page. Background `{colors.canvas}`, text `{colors.ink}` in `{typography.caption}`, rounded `{rounded.pill}`, padding 12px × 16px. Contains a small product thumbnail + label + price delta. Arranged in a grid of 4–5 options per row.
...
**`environment-quote-card`** — A photographic-canvas hero specific to the environment page. Dark photographic backdrop (mountain vista at dawn) with `{colors.surface-tile-1}` as the fallback color, centered white-text headline in `{typography.display-lg}` (40px), small green "2030 sustainability" pictographic logo above the headline, single `{component.button-primary}` below. Padding `{spacing.section}` (80px).

**`floating-sticky-bar`** — Floats at the bottom of the viewport on the flagship product buy page during scroll. Background `{colors.canvas-parchment}` at 80% opacity with `backdrop-filter: blur(N)`, height 64px, padding 12px × 32px. Left: running price total in `{typography.body}`. Right: `{component.button-primary}` ("Add to Bag").
...
- Don't set body copy at weight 500 — the reference design's ladder is 300 / 400 / 600 / 700, with 500 deliberately absent. Body is always 400; strong inline is 600; display is 600.
...
### Collapsing Strategy
- **Global nav**: full horizontal link row on desktop → collapses to the reference design logo + hamburger + bag icon at 834px and below.
...
5. Display headlines stay the display typeface 600 with negative letter-spacing. Body stays the text typeface 400 at 17px. The boundary is unbreakable.
...
- Dark-mode counterparts for store and accessories utility cards were not surfaced on the analyzed pages; the system documented is the daytime/light-dominant variant the reference design ships by default.
