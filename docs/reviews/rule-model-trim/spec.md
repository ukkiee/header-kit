...
- **마이그레이션 — 데이터 손실, 조용히, 검증 전 필터 (R-1):** `kind:'csp'` 수정은 **검증 게이트보다 먼저** 걷어낸다 — 순서가 핵심이다. 두 진입점 모두 "검증이 먼저"라, csp를 무효로 본 채 검증에 닿으면 그것만 빠지는 게 아니라 전체가 버려진다. 그래서 각 경로에서 검증 **이전** 단계에서 csp를 제거한다:
  - **로드(`parseStoredState`):** 프로필이 `isProfile` 검증에 닿기 **전에**(backfill 단계) csp 수정을 제거한다. 안 그러면 `isProfile`이 false를 돌려 `parseStoredState`가 `createDefaultState()`로 **전체 상태를 리셋**한다(프로필 전부 소실 — user story 8).
  - **import(`parseImport`):** `validateProfileEntry`가 돌기 **전에** csp 수정을 제거한다. 안 그러면 파일 전체가 거부된다(user story 9). 정규화(`normalizeImportedProfiles`)에서만 거르면 이미 검증을 지난 뒤라 늦다.
  - 두 경로 모두 **버리되 나머지(다른 수정·프로필 메타)는 보존**한다. import의 기존 "소실 종류 안내(notice)"는 CSP엔 쓰지 않는다(로드와 일치, 사용자 결정) — CSP notice를 추가하지 않는다.
...
- **vitest (core) — 주 시임.** CSP 제거의 핵심을 최상위 순수함수에서 본다.
  - **로드 드롭 (검증 관통, R-1):** 반드시 **`parseStoredState` 자체로** 테스트한다 — 내부 backfill 헬퍼만으로는 "검증 전 필터"가 리셋을 막는지 증명하지 못한다. `kind:'csp'` 수정 + 지원 규칙이 섞인 상태를 넣으면 기본값 **리셋이 아니라** csp만 빠지고 같은 프로필의 다른 규칙·프로필 메타가 온전하다. 선행 예: 기존 persist 테스트(무효 필드 치유·마이그레이션).
  - **import 드롭 (검증 관통, R-1):** 반드시 **`parseImport` 자체로** 테스트한다 — 내부 `normalizeImportedProfiles`만으로는 검증 전 필터 순서를 증명하지 못한다(정규화는 검증 뒤라, 거기서만 걸러도 파일은 이미 거부됨). CSP + 지원 규칙이 섞인 파일 → `ok:true`, csp만 빠지고 나머지 수정·프로필 메타 보존, **CSP에 대한 notice 없음**(기존 소실-종류 notice와 구분). 선행 예: 기존 transfer/import 테스트.
...
- 마이그레이션의 핵심 함정(R-1): 두 경로 다 **검증이 정규화/드롭보다 먼저** 온다. csp를 무효로 본 채 검증에 닿으면 로드는 전체 리셋, import는 파일 전체 거부다. 그래서 필터는 **검증보다 앞**이어야 하고("버리되 나머지 보존"), 테스트는 정규화/backfill 헬퍼가 아니라 **`parseStoredState`·`parseImport` 진입점 자체**로 그 순서를 관통 검증한다.
