...
### 인간 결정 — 티켓 13 R-1 test-touch 승인 2026-07-28T06:36:23Z

`guard-would-trip` 정지에 대한 처분. **사용자가 승인했다.**

**결정.** `src/core/summary.test.ts`의 5줄(:100 :103 :109 :112 :133) **1:1 필드 개명을 명시
승인한다** — `ProfileRowStatus.ruleCount` → `enabledModificationCount`. 순-라인 0의 치환이라
약화가 아니고 컨덕터의 `diff-guard`도 통과하며, 막고 있던 것은 서브에이전트 측
`guard:test-touch`의 "비-순수-추가" 문구뿐이었다. 선례는 티켓 09 fix R-7과 티켓 10 R-3이며,
둘 다 같은 형태로 사람의 명시 승인을 받아 풀렸다.

**집행 방법.** R-1과 R-4는 **각각 별개의 커밋**으로 적용한다. R-1은 3파일 8줄. R-4는 픽스
에이전트가 `.scratch/wide-ui-redesign/tickets/13.md`의 `## fix R-1 R-4`에 그려 둔 "막히지 않는
경로"를 그대로 쓴다 — Pause 아이콘 유지 + 기존 `ariaStatePaused`를 이름과 표식 사이에 + 신규
N41f, 3파일 ≈25줄. 티켓 AC7이 선례로 든 `'II'` 대체 경로는 `smoke.mjs:3850-3853`의 N41e 단언
둘을 깨뜨리므로 **쓰지 않는다.** 각 커밋 뒤에 `diff-guard --mode fix`를 돌린다.

**`max_starts` 8 → 10.** 남은 일(픽스 2건·13 종결·Stage 5·릴리스 라운드 2)은 크래시가 없으면
한 번의 시작에 든다. 2회 여유는 게이트 도중 크래시나 라운드 2 재시도용 마진이다.

**R-4의 판정에 관하여, 기록으로 남긴다.** 픽스 에이전트는 이 행을 `needs-decision`이 아니라
**결함 확정**으로 판정했고, 그 판정은 기준 감사자의 "criterion 2 met"과 정면으로 충돌한다.
근거는 티켓 AC2의 문장 구조다 — 뒤 절이 접근성 이름을 "도"로 따로 열거하므로 앞 절의 "텍스트"가
접근성 이름일 수 없다. 이 승인은 그 판정을 뒤집지 않고 그대로 둔다. 감사자가 met으로 읽은 기준이
리뷰에서 부분으로 뒤집힌 것은 **라운드가 일한 결과**이지 감사자의 과실이 아니다.

**출처.** 사용자가 "승인" 한 마디로 개명을 승인했고, `max_starts` 값은 컨덕터가 10을 권장해
...
`guard:zero-accepts` 정지에 대한 처분. **사용자가 네 가지를 결정했고, 이 줄들을 타이핑한 주체는 컨덕터다** — 이 브랜치의 선례(`862478e`, 티켓 13 R-1)와 같은 형태이며, 재개하는 컨덕터가 사용자가 직접 쓴 것으로 오인하지 않도록 적는다.

**결정 1 — 이 브랜치에서 고친다.** critical 3건(R-1·R-2·R-3)을 티켓으로 분해해 Stage 4 정규 경로로 배선한다. 선례는 릴리스 r1의 R-4 → 티켓 11·12·13이다.

**결정 2 — 티켓 하나로 합본한다.** 사용자가 속도를 이유로 3티켓이 아니라 **1티켓**을 골랐다 → `.scratch/wide-ui-redesign/issues/15-release-r2-criticals.md`. 대가는 기록해 둔다: 서로 무관한 수정 3건이 한 diff에 섞여 리뷰 신호가 흐려지고, 기준 감사가 1건이라도 not-met이면 전체가 재시도된다.

**결정 3 — 어댑터·런타임 주입 시임을 정식 명명한다.** `spec.md` Testing Decisions에 두 줄을 추가했다 — 어댑터 단위(`src/platform/*.test.ts`, fake `browser.storage`)와 런타임 조율(`src/runtime/*.test.ts`, `BackgroundDeps` 주입). **이것이 이 처분의 가장 중요한 항목이다.** 조사가 확인한 바로는, 이 시임 없이는 R-2·R-3의 회귀 테스트를 어디에도 쓸 수 없고 티켓은 "판정 규칙"만 증명하고 "배선"은 증명하지 못한 채 나간다 — 즉 **HEAD에서 실패하는 테스트가 없는 티켓**이 된다. 그것은 이 에스컬레이션이 문제 삼은 바로 그 실패 형태(N43이 경합을 피해 감)를 재생산한다. 세 갈래 중 (b) 정식 명명을 골랐다.

**결정 4 — R-3은 Tier 2(서비스워커 단일 writer)다.** Tier 1 완화(쓰기 직전 재읽기·병합)는 정방향 창만 좁히고 **역방향(지운 행이 손상됨으로 되살아남)은 열린 채 남긴다.** `bk:manifest`의 writer 둘이 서로 다른 JS 컨텍스트(서비스워커 / 렌더러)에 있고 `browser.storage`에 CAS가 없으므로 인프로세스 락으로는 원리적으로 못 고친다. 함께 `spec.md` Out of Scope의 "직렬 writer·자동백업 드레인" 문장이 **sync↔local 이관 트랜잭션에 한정**되고 `bk:` 단일 writer에는 적용되지 않음을 개정으로 명시했다 — 하지 않으면 다음 게이트가 같은 논쟁을 다시 연다.

**결정 5 — `guard:test-touch` 승인.** `scripts/smoke.mjs`의 두 자리를 승인한다: (1) `:4273-4277` N43 주석 정정, (2) 필요 시 `:4390`의 정착 대기 값 상향. **둘 다 단언 0줄 변경이지만 순수 추가가 아니다.** 적대적 비평이 (2)가 원안에서 승인 범위 밖에 있으면서 기준에는 들어가 있던 것을 잡아냈다 — 그대로 뒀으면 구현자가 커밋 직전에 `guard-would-trip`으로 멈췄을 것이다. 선례: 티켓 13 R-1(`5d96fbb`), 티켓 10 R-3.

**결정 6 — 릴리스 라운드 3을 사전 승인한다.** 게이트당 2라운드를 이미 다 썼고 review-gate는 "라운드 3은 사람이 지시하지 않으면 절대 돌리지 않는다"고 정한다. 사용자가 명시 승인했으므로, 티켓 15 종결 → `verification.md` 재작성 → **릴리스 라운드 3(검증 전용, accept 불가)** 까지 무인으로 간다. `max_starts`를 10 → **12**로 올린다.

**티켓 15는 조사·설계·적대적 비평을 거쳐 작성됐고, 비평이 잡은 것들을 기록에 남긴다** — 남기지 않으면 다음 읽는 사람이 티켓 본문만 보고 이 판단들이 어디서 왔는지 모른다.

1. **원안 티켓 17의 core 테스트 근거가 공허했다.** 선례로 든 `reset.test.ts`의 `autoBackupTick`(:81-86)은 `if (world.suspended) return;`으로 시작하는데, 중단-우선 오케스트레이터에서는 첫 효과가 곧바로 `suspended`를 세우므로 **삭제 창 동안 모든 틱이 no-op**이다. 그 하네스로는 "그 사이 커밋된 스냅샷"이 구조적으로 만들어지지 않아, 테스트가 HEAD에서도 픽스 뒤에도 통과한다. 독립된 두 렌즈가 같은 것을 잡았다. → 티켓 15 기준 C-4가 **"이미 진행 중이던(중단 플래그를 이미 통과한) 백업"** 을 명시하고 "HEAD에서 실패한다"를 기준에 박았다.
2. **`backupSuspended`는 카운팅 세마포어가 아니다**(`background-bootstrap.ts:155`, 단순 boolean, 현재 유일 사용자 `fullReset`). 삭제를 두 번째 사용자로 그냥 얹으면 먼저 끝난 쪽이 플래그를 되돌려 **새 경합이 생긴다.** → 기준 C-2.
3. **`Promise<boolean>`은 `Promise<void>`에 대입 불가**(`error TS2322`, 비평가가 이 저장소의 tsc로 실증). R-2 조사가 "assignment-compatible"이라고 쓴 것은 **사실 오류**다. → 기준 B-3이 `BackgroundDeps.persistState` 시그니처를 그대로 두고 CAS를 별도 함수에 두게 했다.
4. **삭제는 `storage.local.state`를 건드리지 않아 `onStateChanged`가 뒤따르지 않는다** — 전체 초기화가 중단을 복구하는 경로가 삭제에는 없으므로, 재개가 무조건 재예약하지 않으면 그 백업은 영구 소실된다. → 기준 C-3.
5. **CAS의 자리가 창을 닫는다.** `commitMigration`이 따로 한 번 더 읽는 구현은 `persistState`의 get(:72)과 set(:78) 사이 창을 그대로 남긴다. 창을 닫는 것은 재읽기가 아니라 **읽기와 쓰기 사이에 await가 없다는 사실**이다. → 기준 B-1.
6. **리뷰어 권고문을 글자 그대로 구현하면 기존 테스트가 깨진다.** "every alternative at every nesting level"을 순진한 분할로 읽으면 `^https://(ads|cdn)\.example\.com/`가 찢어져 `url-scope.test.ts:120`이 뒤집힌다. 올바른 해법은 **문맥 분배 전개**다. → 기준 A-2. 조사자가 두 구현을 실제로 돌려 확인했다.

**조사가 리뷰어를 교정한 것도 남긴다.** R-2·R-3은 실재하나 리뷰어가 심각도를 다소 과장했고, 반대로 R-3에는 **리뷰어가 놓친 역방향 경합**(지운 행이 손상된 채 되살아남)이 있어 Tier 1이 부적절함을 확정했다. R-1은 리뷰어가 오히려 과소평가했다 — 오판정 모양이 하나가 아니다.

### ticket 15 code-review r1 — auto-triage
_policy CR-1 · feature-loop/policies/ticket-review-cr1.md · sha256 27ad2f0313d78a9b · decided 2026-07-28T09:37:16Z · fixed point 37b3c49fedb656241a51e2b1b95527d84706ef37 · ticket .scratch/wide-ui-redesign/issues/15-release-r2-criticals.md_

R-1 [AUTO CR-1 cr:smell] defer — Standards: 새 시임이 `Promise<unknown>`이라 `DeleteSnapshotResult`가 경계에서 지워진다; -/-; src/runtime/background-bootstrap.ts:63; res:none; Standards 축이 스스로 "judgement, not hard"로 한정했다 — 브랜치 규율 "종류·상태가 늘면 타입이 먼저 깨지게 한다"에 대한 연성 위반이고, 그 규율이 적힌 review-brief.md는 gitignore된 루프 작업파일이지 저장소가 문서화한 표준이 아니다. `cr:standard`는 문언이 "하드 위반"으로 한정돼 있어 쓰지 않고, 잔여 규칙("어떤 규칙에도 안 맞으면 판단 호출로서 cr:smell")을 따른다. 참고: bootstrap이 그 값을 실제로 들여다보지 않는다는 리뷰어의 관찰이 연성 판정의 근거다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-1 (R-12와 동일 결함, 다른 축)
R-2 [AUTO CR-1 cr:smell] defer — Standards: `onSnapshotDeleteRequest`가 `onCommand`의 리스너 봉투를 그대로 재생산한다 (Duplicated Code); -/-; src/platform/stateStore.ts:-; res:none; 타입가드·`void handler(...).then(sendResponse).catch(...)`·`return true`·`return undefined`가 문자 그대로 겹친다. 메시지 채널 둘에 모양 하나 — 공유 `onTypedMessage`가 둘을 다 나를 수 있다. 베이스라인 스멜이므로 정의상 판단 호출이다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-2
R-3 [AUTO CR-1 cr:smell] defer — Standards: `MAX_EXPANDED_BRANCHES`가 세 자리에서 각각 강제된다 (Duplicated Code); -/-; src/core/url-scope.ts:-; res:none; `concatBranches`와 `done.length > MAX_EXPANDED_BRANCHES` 두 자리. 테스트 쪽도 같은 모양이 둘 — `installDeferredStorage`(backupStore.test.ts)와 `seedDeferredLocal`(stateStore.test.ts)는 다음 `get`을 붙잡는 동일한 fake다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-3
R-4 [AUTO CR-1 cr:smell] defer — Standards: `verifySnapshotDeleteComplete`가 기존 `verifySnapshotDeleted` 옆에서 어느 쪽이 넓은 술어인지 못 알린다 (Mysterious Name); -/-; src/core/backup.ts:-; res:none; 의미를 doc 주석이 전부 지고 있다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-4
R-5 [AUTO CR-1 cr:smell] defer — Standards: `literal(text)`가 `[text]` 위에 아무것도 얹지 않는다 (Middle Man); -/-; src/core/url-scope.ts:-; res:none; 호출부 넷 전부 배열 리터럴로 족하다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-5
R-6 [AUTO CR-1 cr:smell] defer — Standards: `platform/stateStore.ts`가 백업 스냅샷 사유로도 바뀌게 됐다 (Divergent Change); -/-; src/platform/stateStore.ts:-; res:none; `requestSnapshotDelete`/`onSnapshotDeleteRequest`가 `BackupTarget`·`DeleteSnapshotResult`를 끌어와 상태 저장소에 두 번째 무관한 변경 사유가 생겼다. `backupStore` 쪽 메시징 쌍이면 갈라 둘 수 있다. **이 항목은 R-1·R-12와 같은 설계 지점을 다른 각도에서 본 것**이므로 후속 처리는 함께 판단해야 한다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-6
R-7 [AUTO CR-1 cr:smell] defer — Standards: `url-scope.ts`에 포기 채널이 둘이다; -/-; src/core/url-scope.ts:-; res:none; `concatBranches`는 `GIVE_UP`/`Branches`로, `expandAlternatives`는 맨 `null`로 포기한다. 리뷰어가 스스로 minor로 표시했다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-7
R-8 [AUTO CR-1 cr:smell] defer — Spec: C-8의 정착 대기 실측에 아티팩트가 없다; -/-; scripts/smoke.mjs:4423; res:none; 기준은 "정착 대기가 충분한지 **실측**하고 모자라면 그 값만 올린다"이고, 주석 재작성은 착지했으나 `8000`은 그대로이며 실측 흔적이 diff에 없다. 구현자 저널도 "실측 자체는 diff에 흔적이 남지 않아 값 불변만 확인함"이라 시인한다. cr:defect를 검토했으나 그 문언은 "잘못된 동작·깨진 불변식·기준이 함의하는 미처리 케이스"이고 여기서 코드 동작은 틀리지 않았다 — 증거 공백이지 결함이 아니다. **반대 방향 증거를 기록에 남긴다: 스모크가 서로 독립된 두 실행(구현자, 그리고 컨덕터 자신)에서 129/129로 통과했고, `8000`이 모자랐다면 그 자리가 흔들렸을 것이다.** 그럼에도 리뷰어의 지적("증명하지 못하는 green을 이미 한 번 출하했기 때문에 이 티켓이 존재한다")은 절차로서 옳다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-8
R-9 [AUTO CR-1 cr:smell] defer — Spec: R-3 드레인의 red 증거가 동작 실패가 아니라 새 표면이다; -/-; src/runtime/background-bootstrap.test.ts:394; res:none; 새 런타임 테스트 셋은 `deps.deleteBackupSnapshot`/`deps.onSnapshotDeleteRequest`를 몬다 — 37b3c49에 없던 심볼이라 컴파일에 실패할 뿐 동작에 실패하지 않는다. **리뷰어 스스로 "partial, not absent"로 한정했고**, R-3의 진짜 red는 `src/platform/backupStore.test.ts:112-145`가 실제로 뒤집히는 것으로 서 있다 — 기준 감사자가 독립적으로 지목한 위치(backupStore.test.ts:126)와 같다. 기준 1은 그 어댑터 시임으로 충족되고 런타임 셋은 추가 피복이므로 미충족이 아니다. follow-up docs/reviews/wide-ui-redesign/followups.md#T15-R-9
...
### 인간 결정 — 릴리스 r3 처분: 브랜치 정지 2026-07-28T13:31:18Z

`guard:zero-accepts` 정지에 대한 처분. **사용자가 결정했고, 이 줄들을 타이핑한 주체는
컨덕터다** — 이 브랜치의 선례(`862478e`, 그리고 2026-07-28T08:1xZ 결정)와 같은 형태이며,
나중에 읽는 사람이 사용자가 직접 쓴 것으로 오인하지 않도록 적는다.

**결정 — 이 브랜치를 여기서 멈춘다.** 릴리스 r3의 critical 3건(R-1 `url-scope.ts:38`,
R-2 `stateStore.ts:106`, R-3 `background-bootstrap.ts:294`)은 **이 브랜치에서 고치지
않는다.** 세 건은 별도 작업으로 떼어내 `/grilling` → `/to-spec`부터 새 슬러그로 다시
시작한다. 사용자에게 세 선택지(A 여기서 멈춤 · B 이 브랜치에서 티켓으로 재시도 + 라운드 4
사전 승인 · C R-1만 고치고 R-2·R-3 이월)를 근거와 대가와 함께 제시했고 **A**를 골랐다.

근거 넷:

1. **같은 결함의 3회차다** — r2가 지목 → 티켓 15가 고침 → r3가 여전히 열려 있다고 판정.
   이 루프는 이 세 건을 두 번 닫으려다 두 번 실패했다.
2. **리뷰어 권고가 전부 구조 변경이다.** R-2는 마이그레이션과 커맨드의 read-modify-write를
   같은 FIFO 배리어에 넣거나 진짜 revision/CAS를 도입하라 하고, R-3은 모든
   `bk:manifest` 변이 — 자동백업·스냅샷 삭제·`clearCloudBackups`·전체 초기화 — 를 하나의
   서비스워커 FIFO로 보내라 한다. 게이트 픽스 한도(3파일/80줄)에 들어갈 크기가 아니고,
   티켓 15 같은 합본 티켓으로도 위험하다.
3. **R-3 권고는 `spec.md`의 Out of Scope를 정면으로 다시 연다** — "직렬 writer·자동백업
   드레인"이 sync↔local 이관에만 한정된다고 2026-07-28T08:1xZ에 개정했는데, r3의 권고는
   그 경계 자체를 옮기라는 요구다. 그건 Stage 1–2에서 사람과 할 결정이지 Stage 4 티켓이
   삼킬 결정이 아니다.
4. **브랜치는 push된 적도 PR도 없다.** 멈춰도 잃는 것이 없다.

**대가를 명시한다:** 이 브랜치는 릴리스 게이트를 통과하지 못한 상태로 남는다. 머지하려면
세 건이 먼저 닫혀야 한다. 새 작업은 `/grilling` → `/to-spec`부터 다시 시작한다.

**`/feature-loop`를 이 슬러그에서 다시 진입시키지 말 것.** `ESCALATION.md`의 재무장
규칙(P8) 때문에 처분을 `Resolved:` 줄로 적었지만 그 뜻은 "재개"가 아니라 "정지"다.
지금 재진입하면 열린 티켓이 0이므로 곧장 Stage 5로 들어가 **승인된 적 없는 릴리스 라운드
4를 띄우려 한다.** 라운드 4는 사람의 별도 명시 승인 전에는 돌지 않는다.

**이 브랜치가 남기는 상태(사실 기록):** 티켓 01–15 전부 종결. HEAD에서 스위트 green —
`bun run build && bun run test && bun run smoke` exit 0, Tests 390/390, smoke 129/129,
`tsc --noEmit` 0. `verification.md`는 `Verified-SHA: aaeebc8`로 stale이 아니다.
미해결 critical 3건과 `followups.md`의 이월 항목 40건(티켓 15가 더한 11건 포함)이 열려 있다.
