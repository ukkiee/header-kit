...
> **정정 2026-07-28 (릴리스 게이트 r2 R-2로 반증됨).** 위 "티켓 14는 **동시 writer를 없애**
> 그 창을 닫는다 … 지금은 '쓰기 경로가 하나뿐'이라는 배치상의 성질이 유일한 보호막이다"는
> 전제가 **틀렸다.** 그 배치상의 성질은 그때도 성립하지 않았다 — 커맨드 리스너는 마이그레이션
> 커밋보다 **먼저** 등록되고(`src/runtime/background-bootstrap.ts:262` vs :294),
> `commitMigration`(`src/platform/stateStore.ts:42-49`)은 커맨드 FIFO(`src/runtime/executor.ts`)
> 밖에서 쓴다. 즉 티켓 14는 writer를 없앤 것이 아니라 **하나 만들었고**, 이 항목이 예언한
> "새 writer가 하나라도 더 생기면 같은 창이 다시 열린다"는 이미 실현돼 있었다. 릴리스 r2의
