...
**이 문서는 세 번 갱신됐다.** (1) 릴리스 게이트 r1의 R-1 반영 뒤, (2) 게이트 통과 후 들어온 UI
다듬기 묶음 뒤, (3) 그 다듬기에 대한 릴리스 게이트 r3의 R-2 반영 뒤. 커밋이 붙을 때마다 앞선
증거는 그 트리를 더 이상 가리키지 않으므로, 그때마다 전 스위트를 다시 돌려 아래 SHA를 바꿨다.

## 못박은 지점

| 항목 | 값 |
|---|---|
| Verified SHA | `3b9e5f601b6d17789209a2692e6ff73fab48aada` |
| Verified tree | `9b4c952362865de9ce11a337479473fb6cd79e72` |
...
이 문서 자체의 커밋은 위 SHA **다음**에 온다. 검증 대상은 `3b9e5f6`의 트리이고, 이후 추가되는
