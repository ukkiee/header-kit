...
**이 문서는 네 번 갱신됐다.** (1) 릴리스 게이트 r1의 R-1 반영 뒤, (2) 게이트 통과 후 들어온 UI
다듬기 묶음 뒤, (3) 그 다듬기에 대한 릴리스 게이트 r3의 R-2 반영 뒤, (4) 착지 과정에서 드러난
스모크 타이밍 흔들림 세 곳(F1·N23a·I1)을 안정화한 뒤. 커밋이 붙을 때마다 앞선 증거는 그
트리를 더 이상 가리키지 않으므로, 그때마다 전 스위트를 다시 돌려 아래 SHA를 바꿨다.

**착지 직전 상태 유의:** (4)의 스모크 안정화는 릴리스 게이트 r4 approve **이후**의 변경이지만
`scripts/smoke.mjs`만 건드린 **테스트 하니스 신뢰성 개선**이다 — 제품 코드 0, 단언 의도 불변
(관측을 폴링/프레임 수집으로 바꿨을 뿐 무엇을 검증하는지는 그대로). 그래서 릴리스 approve를
다시 게이트하지 않고 이월했다. 근거·경위는 `decisions.md`의 `### landing waiver`.

## 못박은 지점

| 항목 | 값 |
|---|---|
| Verified SHA | `b5a53195a59960c8e0e7938e3f6d57306cf26ee8` |
| Verified tree | `0b5926765ff63e29c404c11b9541bc41a9e064b5` |
...
이 문서 자체의 커밋은 위 SHA **다음**에 온다. 검증 대상은 `b5a5319`의 트리이고, 이후 추가되는
...
| 5 | `bun run smoke` | 0 | `106/106 passed` (FAIL 0줄) |
...
```
bundle gate: popup 즉시 로드 합계 521.2KB = baseline 386KB + 135.2KB (한도 +143KB) — PASS
...
| **게이트 R-2** | 스크롤바 페이드가 reduced-motion에서 꺼짐(값은 유지) | `bun run smoke` | `N33` 기본 prop=`opacity` / reduced prop=`none`, opacity 0.6 양쪽 유지 |
...
| `N33` 스크롤바 페이드가 reduced에서 꺼짐 | `motion-reduce:transition-none` 제거 | reduced에서도 `transition-property=opacity` 유지 — **FAIL** |
