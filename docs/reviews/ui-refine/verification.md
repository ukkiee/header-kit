...
- 07 삭제 Undo: N20a/b (Placeholder 값 보존 원자 복원·미클릭 유지) — **이 결정은 뒤집혔다** (ADR 0017 재개정): 규칙 삭제가 2단 확인이 되면서 Undo 토스트와 `restore-modification`이 함께 퇴역했고, N20a/b는 새 규약(첫 클릭은 되묻기만 · 되물음은 행마다 따로)을 잰다. **아래 수치와 판정은 그 시점의 것이고 고치지 않는다** — 이 문서는 그때 무엇을 어떻게 쟀는지의 기록이지 지금의 계약이 아니다.
