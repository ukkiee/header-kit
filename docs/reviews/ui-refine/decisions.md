...
- **07 '규칙 삭제 → Undo 토스트'가 뒤집혔다.** 파괴적 동작 다섯을 2단 확인으로 통일하면서
  토스트와 `restore-modification`, 그리고 release r1 R-1이 다루던 "삭제 후 Undo가 고아
  materialized를 복원한다"는 자리 자체가 사라졌다. 근거와 새 규약은 ADR 0017의 재개정 절이
  갖는다 — 이 파일은 그때의 게이트 판정 기록이라 위 항목들은 고치지 않는다.
