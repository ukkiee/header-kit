 ...
 * (`12 rules · paused`)가 이름을 예닐곱 자로 눌러 버린다 — 목록에서 프로필을 짚는 단서가
 * 곧 그 이름인데 그것이 먼저 잘리는 셈이다. 두 줄로 나누면 열도 넓어지지 않고 이름도 남는다.
 *
 * **아래 줄로 뺐어도 그 칸은 87px뿐이다.** 꺼짐 상태의 en 낱말이 `not applied`에서 `off`로
 * 돌아온 것이 그래서다 — 앞의 것은 107px을 요구해 늘 잘려 있었다(`i18n.ts`의 그 주석).
