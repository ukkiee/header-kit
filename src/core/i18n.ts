...
const en = {
  ...
  resume: 'Resume',
  /**
   * 토스트 제목은 **한 줄에 들어가야 한다** — 넘치면 말줄임표로 잘리고, 잘린 뒤 남는 것이
   * "Paused"뿐이면 그건 이미 배지와 버튼이 말한 것이다. 가용 폭 230px에 en은 40자·ko는
   * 27자가 들어간다(실측). 예전 문구는 en 47자·ko 32자라 둘 다 잘렸다.
   */
  pausedNote: 'Paused — no modifications applied.',
...
export const MESSAGES: Record<Locale, Record<MessageKey, string>> = {
  ...
  ko: {
    ...
    pause: '일시정지',
    resume: '재개',
    pausedNote: '일시정지 중 — 수정이 적용되지 않아요.',
    newProfile: '새 프로필',
    profiles: '프로필',
    ...
    scopeAllUrls: '모든 URL',
    noRulesYet: '아직 규칙이 없어요. 아래에서 추가해 보세요.',
    ...
    emptyMarker: '(비어 있음)',
    saveRejected: '저장하지 못했어요.',
    urlFilterScope: 'URL 필터 (이 규칙에만 적용)',
    matchRegex: '정규식',
    modCookie: '요청 쿠키',
    modSetCookie: '응답 쿠키',
    modRedirect: '리다이렉트',
    redirectCaptureNote: '패턴의 캡처 그룹 \\1–\\9를 이동할 URL에서 다시 쓸 수 있어요.',
    blockNote: '조건에 맞는 요청을 차단해요. 무엇을 막을지는 위 URL 필터가 정해요.',
    unsupportedPattern: '이 패턴으로는 브라우저가 규칙을 만들지 못해요.',
    export: '내보내기…',
    import: '가져오기…',
    exportAction: '내보내기',
    importAction: '가져오기',
    ...
    lastBackupNever: '{store}에는 아직 백업이 없어요.',
    storeCloud: '브라우저 계정',
    storeLocal: '이 브라우저',
    preferences: '환경설정',
    add: '추가',
    ...
    retirementNoticeRules:
      '더 이상 지원하지 않는 조건을 지웠어요. 규칙 {count}개가 전보다 넓은 범위에 적용돼요.',
    retirementNoticeRule:
      '더 이상 지원하지 않는 조건을 지웠어요. 규칙 {count}개가 전보다 넓은 범위에 적용돼요.',
    ...
    rulesCouldNotApply: '규칙을 적용하지 못했어요:',
    noBackupsYet: '아직 백업이 없어요. 프로필을 바꾸면 그때 하나 만들어져요.',
    noProfilesYet: '아직 프로필이 없어요. 아래 + 새 프로필을 눌러 만들어 보세요.',
    corrupt: '손상됨',
    theme: '테마',
    themeSystem: '시스템',
    themeDark: '다크',
    ...
    badgeCountNote: '툴바 아이콘에 지금 적용 중인 규칙 수를 표시해요.',
    cloudSync: '클라우드 동기화',
    cloudSyncOn: '켜짐. 새로 만드는 백업은 브라우저 계정에 저장돼요.',
    cloudSyncOff: '꺼짐. 새로 만드는 백업은 이 브라우저에만 저장돼요.',
    cloudBackupsPresent: '클라우드에 백업이 남아 있어요.',
    cloudBackupsNone: '클라우드에 백업이 없어요.',
    cloudBackupsUnknown: '클라우드에 백업이 남아 있는지 확인할 수 없어요.',
    cloudSyncKeepsHistory: '스위치를 바꿔도 이미 만든 백업은 옮겨지지 않고 처음 저장된 곳에 그대로 남아요.',
    deleteCloudBackups: '클라우드 백업 삭제',
    confirmDeleteCloudBackups: '클라우드에서 지울까요?',
    cloudBackupsDeleted: '클라우드 백업을 지웠어요.',
    cloudDeleteFailed: '클라우드 백업을 지우지 못했어요',
    cloudDeleteRemaining: '클라우드에 백업 키 {count}개가 남아 있어요.',
    confirmDeleteBackup: '이 백업을 지울까요?',
    snapshotDeleteFailed: '이 백업을 지우지 못했어요',
    snapshotDeleteRemaining: '이 백업의 키 {count}개가 아직 남아 있어요.',
    resetEverything: '전체 초기화',
    confirmResetEverything: '전부 지울까요?',
    resetEverythingNote: '이 브라우저와 클라우드의 모든 프로필·선호값·백업을 지워요. 되돌릴 수 없어요.',
    resetRetryNote:
      '한 단계가 실패해도 이미 지운 것은 돌아오지 않아요. 다시 누르면 남은 단계부터 이어서 진행해요.',
    resetDone: '모두 기본값으로 되돌렸어요.',
    resetFailed: '초기화를 끝내지 못했어요',
    resetStoppedAtLocalBackups: '이 브라우저의 백업을 지우는 단계에서 멈췄어요',
    resetStoppedAtSyncBackups: '클라우드 백업을 지우는 단계에서 멈췄어요',
    resetStoppedAtState: '프로필·선호값을 기본값으로 되돌리는 단계에서 멈췄어요',
    resetStoppedAtSummary: '세션 요약을 지우는 단계에서 멈췄어요',
    incognitoBlocked:
      '시크릿 창에서 아직 켜지지 않았어요. 시크릿 트래픽을 수정하려면 확장 상세 페이지에서 “시크릿 모드에서 허용”을 켜 주세요.',
    responsePanelNote:
      '응답 헤더 변경은 DevTools 네트워크 패널에 안 보일 수 있지만 페이지에는 그대로 반영돼요.',
    placeholderNote: '프로필을 켤 때마다 새 값이 만들어지고, 끌 때까지 그 값이 그대로 쓰여요.',
    rawCookieNote: '예전 버전이 저장한 그대로 남아 있는 줄이에요. 위 칸을 채우면 이 줄 대신 그 값이 나가요:',
    dropExportHere: '내보낸 파일을 여기에 끌어다 놓거나, 눌러서 골라 주세요',
    importFileChosen: '고른 파일: {name}',
    importInvalidJson: '올바른 JSON 파일이 아니에요.',
    importNewerFormat:
      '더 새 버전의 HeaderKit이 내보낸 파일이에요(포맷 v{found}). 이 버전은 v{readable}까지 읽어요.',
    importNotExportFile:
      'HeaderKit이 내보낸 파일로 보이지 않아요. 그 파일에는 "headerkit" 버전(v{readable} 이하)과 "profiles" 목록이 들어 있어요.',
    importEntryNotObject: '{where}: 이 항목이 객체가 아니에요.',
    importFieldNotText: '{where}: "{field}"는 문자열이어야 해요.',
    importBadColor: '{where}: "color"는 #rrggbb 형태여야 해요.',
    importActiveNotBoolean: '{where}: "active"는 true 또는 false여야 해요.',
    importModificationsNotList: '{where}: "modifications"는 목록이어야 해요.',
    importUnreadableRule: '{where}: {index}번째 규칙을 이 버전이 읽지 못해요.',
    importFiltersNotList: '{where}: "filters"는 목록이어야 해요.',
    importUnreadableLegacyFilter: '{where}: {index}번째 옛 필터를 이 버전이 읽지 못해요.',
    importFiltersMoved: '"{name}": 프로필에 걸려 있던 옛 필터를 각 규칙으로 옮겼어요.',
    importDroppedLostFilter:
      '"{name}": 규칙 조건으로 옮길 수 없는 옛 필터 {count}개를 버렸어요(제외 URL·탭·그룹·창).',
    importDroppedLostFilters:
      '"{name}": 규칙 조건으로 옮길 수 없는 옛 필터 {count}개를 버렸어요(제외 URL·탭·그룹·창).',
    importDroppedDisabledFilter: '"{name}": 꺼져 있던 옛 필터 {count}개를 버렸어요.',
    importDroppedDisabledFilters: '"{name}": 꺼져 있던 옛 필터 {count}개를 버렸어요.',
    importRuleLostConditions:
      '"{name}": 규칙 {count}개가 이 버전이 더는 지원하지 않는 조건을 잃어 전보다 넓은 범위에 적용돼요(제외·요청 출처·탭 도메인, 자동 해제, HEAD/CONNECT/OTHER 메서드).',
    importRulesLostConditions:
      '"{name}": 규칙 {count}개가 이 버전이 더는 지원하지 않는 조건을 잃어 전보다 넓은 범위에 적용돼요(제외·요청 출처·탭 도메인, 자동 해제, HEAD/CONNECT/OTHER 메서드).',
    warnEmptyHeaderName: '헤더 이름 없음',
    warnEmptyHeaderNameDetail: '헤더 이름이 비어 있어 이 수정을 건너뛰었어요.',
    warnHeaderOverlap: '프로필 간 헤더 겹침',
    warnHeaderOverlapDetail: '여러 활성 프로필이 "{header}"를 수정해요. 목록에서 위에 있는 프로필이 이겨요.',
    warnRegexTooLong: 'URL 패턴 길이 초과',
    warnRegexTooLongDetail: 'URL 패턴이 {limit}자를 넘어 이 수정을 건너뛰었어요.',
    warnQuotaExceeded: '규칙 수 한도 초과',
    warnQuotaTotalDetail: '세션 규칙 한도({limit})를 넘어 일부 수정이 적용되지 않았어요.',
    warnQuotaRegexDetail: '정규식 규칙 한도({limit})를 넘어 일부 수정이 적용되지 않았어요.',
    warnMissingMaterialization: 'Placeholder 값 없음',
    warnMissingMaterializationDetail:
      '활성 프로필의 Placeholder에 아직 값이 없어 그 프로필 전체를 규칙에서 제외했어요.',
    warnAppendNotAllowed: '헤더 덧붙이기 불가',
    warnAppendNotAllowedDetail: '요청 헤더 "{header}"는 덧붙일 수 없어 덮어쓰기로 대신했어요.',
    warnBlockWithoutScope: 'URL 필터 없는 차단 규칙',
    warnBlockWithoutScopeDetail: '차단 규칙에 URL 필터가 없으면 모든 요청이 막히므로 이 규칙을 건너뛰었어요.',
