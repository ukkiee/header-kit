...
import { POPUP_FADE_S, POPUP_SPRING_EASE } from './motion-tokens';

/**
 * CSS가 소유하는 전이의 길이·곡선을 문서 루트에 변수로 올린다.
 *
 * **왜 필요한가.** 마운트를 Base UI가 소유하는 표면(Select 팝업)은 motion으로
 * 감쌀 수 없어 CSS 전이를 쓴다. 그런데 그 값을 컴포넌트에서 인라인 `style`로 주면 Base UI가
 * 자기 CSS 변수를 쓰면서 **덮어써 사라진다**(실측). 그렇다고 Tailwind
