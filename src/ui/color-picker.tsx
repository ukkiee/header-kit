...
function ColorPickerContent(props: React.ComponentProps<typeof PopoverContent>) {
  ...
  if (context.inline) {
    /*
     * **로컬 패치 — 업스트림(diceui base-nova) 타입 결함.**
     *
     * 이 함수의 props는 `React.ComponentProps<typeof PopoverContent>`이고 그것은 Base UI의
     * `Popover.Popup.Props`라, `style`이 **상태를 받는 함수**일 수 있다
     * (`(state: PopoverPopupState) => CSSProperties | undefined`). inline 분기는 그 props를
     * 평범한 `div`에 그대로 펼치는데 `div`의 `style`은 `CSSProperties`뿐이라 타입이 맞지 않는다
     * (`check` 게이트가 TS2322으로 잡았다).
     *
     * inline 모드는 Positioner·Popup을 쓰지 않으므로 상태 기반 style을 적용할 자리가 애초에
     * 없다 — 함수 꼴은 버리고 객체 꼴만 넘긴다. 동작은 바뀌지 않는다(함수 style을 이 모드에
     * 넘기는 호출자는 지금 없고, 넘겼다면 그전에도 적용되지 않았다).
     *
     * `shadcn add`로 재복사하면 이 패치가 사라지고 `check`가 곧바로 빨강이 된다 — 즉 이
     * 수정이 조용히 유실되지는 않는다. 그것이 ADR 0014의 "소스를 고치지 않는다"를 어기면서도
     * 감당할 수 있는 이유다.
     */
    const { style, ...inlineProps } = popoverContentProps;
    return (
      <div
