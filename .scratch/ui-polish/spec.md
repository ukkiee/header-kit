...
- **bundle-gate** — ScrollArea·Autocomplete 추가분이 한도 안인지 측정한다. **초과 시 한도를 다시 올리지 않는다** — 먼저 지연 로드나 대안(예: Autocomplete 대신 기존 datalist 유지)을 검토하고, 그래도 불가하면 사용자 재트리아지를 거친다. 이 게이트는 바이트만 재므로 story 31의 근거가 되지 못한다 — 런타임 지연은 위 ui-diag 시작 지표가 담당한다.
  - **재트리아지 1회 발생 (티켓 02, 한도 +120KB → +135KB).** 위 사다리를 순서대로 밟았고(지연 로드 불가 → 대안이 구현 결정과 충돌 → 사용자 승인), 근거는 ui-diag 시작 지표에 회귀가 없다는 **실측**이었다. 측정치·경위의 정본은 `.scratch/ui-polish/issues/02-scroll-area.md` 하나다 — 숫자를 여기 옮겨 적지 않는다.
...
- bundle-gate 여유가 얇다(티켓 02 시점에 한도 +135KB, 사용 +129.5KB). ScrollArea와 Autocomplete를 도입하는 티켓에서 각각 측정치를 기록해, 어느 쪽이 한도를 압박하는지 드러나게 한다. 같은 티켓에서 ui-diag 시작 지표(첫 렌더·상호작용 준비 중앙값)도 함께 기록한다 — 바이트와 지연이 항상 같이 움직이지는 않으므로, 둘을 나란히 남겨야 어느 도입이 무엇을 압박했는지 사후에 갈라낼 수 있다.
