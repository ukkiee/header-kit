...
2026-07-15, 사용자 제공 디자인 스펙(저장소 밖 untracked 리서치 원본)의 토큰을 UI에 적용 — commit `7795efa`. 03의 cva DS를 그릇으로 사용:
- `@theme`(global css)로 앱이 쓰는 색·radius·폰트 토큰을 스펙 값으로 재정의 → 컴포넌트 무수정 전체 리테마. accent #0066cc(focus #0071e3), ink #1d1d1f, parchment #f5f5f7, hairline #e0e0e0, near-black dark 램프, 유틸 radius 8px, 시스템 폰트 스택(SF 우선), 단일 accent·무그림자.
- Button 주 액션 pill + `active:scale-95`, Chip pill 문법.
- **결정**: 밀도 유지 토큰 리스킨(마케팅 레이아웃 미채택). 브랜드 중립 — 리서치 원본 미커밋, 코드·커밋 중립 표현.
- **이후 슬라이스 04~06은 이 테마 위에서 프리미티브를 구축**(behavior-preserving 아님 — 새 디자인 반영).
