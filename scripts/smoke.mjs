...
try {
  ...
  record('A1: 활성 Profile의 헤더가 실요청에 적용', headers['x-headerkit-smoke'] === 'ok',
  ...
  record('A2: Profile off 시 헤더 즉시 제거', headers['x-headerkit-smoke'] === undefined,
    `x-headerkit-smoke=${headers['x-headerkit-smoke']}`);
  ...
  record('C3: 5,000 규칙 전량 교체(제거+재등록)', true, `${replaceMs}ms`);

  const t2 = Date.now();
  await applyExp([]);
  record('C4: 전량 제거로 원상복구', true, `${Date.now() - t2}ms`);
