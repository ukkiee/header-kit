...
**Compile**:
활성 Profile 전체를 브라우저가 적용하는 선언적 네트워크 규칙 집합으로 변환하는 것. 규칙 상태는 항상 저장된 Profile과 주어진 환경(열린 탭 스냅샷, 현재 시각)의 순수 함수다.
_Avoid_: sync, refresh

**Placeholder**:
Modification 값 안에서 `{{uuid}}`처럼 쓰여 Compile 시점에 한 번 실제 값으로 치환되는 토큰. 요청마다 재평가되지 않는다.
