...
**증표**:
Writer Lane을 쥐고 있다는 표시 — 줄이 작업에게 건네주는 것 말고는 만들 수 없다 (ADR 0016, 타입 이름 `Held`). 줄 안에서만 불려야 하는 쓰기 함수가 이것을 인자로 요구하므로, 줄 밖 호출은 컴파일이 막는다. 중첩되는 경로는 다시 쥐지 않고 받은 증표를 그대로 넘긴다.
_Avoid_: 락 (쥐고 놓는 물건이 아니라 자리의 증거다), 권한·permission (사용자 권한과 혼동)
